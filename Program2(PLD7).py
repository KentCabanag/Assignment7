...
user = input("\n\33[33;1mEnter your \33[32;1mPASSWORD\33[0m: ")
...
def validator():
    ...
    if len(user) < 16:
        print("\33[31;1mPassword length should be Greater than 15 letters.")
        val = False
    
    if not any(char.isdigit() for char in user):
        print("\33[31;1mPassword should have at least one number.")
        val = False
    
    if not any(char.isupper() for char in user):
        print("\33[31;1mPassword should have at least one capital letter.")
        val = False

    if not any(char in spclChar for char in user):
        print("\33[31;1mPassword should have at least one special character.\33[0m")
        val = False
